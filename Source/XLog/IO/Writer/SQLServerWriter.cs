...
namespace Extended.Log.IO
{
    ...
    /// 将日志写入到SQLServer
    /// </summary>
    internal class SQLServerWriter
    {
        private static SQLServerWriter instance = new SQLServerWriter();
        public static SQLServerWriter getIns() { return instance; }
        ...
        private void saveLogQueue(ConcurrentQueue<StrongBox<LogModel>> logQueue)
        {
            ...
            if (count > LogPerformance.maxProcessCountPerTimeBySQLServerType) count = LogPerformance.maxProcessCountPerTimeBySQLServerType;
            ...
            while (true)
            {
                ...
                {
                    ISystemLogSync logSync = new ISystemLogSync(Database.Entities.Lib.DBSwitch.DBVersion.SqlServer);
                    logSync.bulkSave(systemLogModels);
