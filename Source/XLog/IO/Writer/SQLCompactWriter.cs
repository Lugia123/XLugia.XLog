...
namespace Extended.Log.IO
{
    ...
    /// 将日志写入到SQLCompact
    /// </summary>
    internal class SQLCompactWriter
    {
        private static SQLCompactWriter instance = new SQLCompactWriter();
        public static SQLCompactWriter getIns() { return instance; }
        ...
        private void saveLogQueue(ConcurrentQueue<StrongBox<LogModel>> logQueue)
        {
            ...
            if (count > LogPerformance.maxProcessCountPerTimeBySQLCompactType) count = LogPerformance.maxProcessCountPerTimeBySQLCompactType;
            ...
            while (true)
            {
                ...
                {
                    ISystemLogSync logSync = new ISystemLogSync(Database.Entities.Lib.DBSwitch.DBVersion.SqlServerCompact);
